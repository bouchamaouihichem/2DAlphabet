## From https://github.com/bouchamaouihichem/2DAlphabet/blob/master/htoaato4b_mctoy.py
...
if CAT.startswith('VBFjj'):
    #PATH    = 'plots/2DAlphabetfiles_VBF_inputs/VBFLo[Hi]_Xto4bv2/'
    SIGS    = ['VBFH']
...
def _get_nparams(fit_poly):
    for j in reversed(range(99)):
        if '@'+str(j) in fit_poly:
            nparams = j+1
            return nparams
    print('\nNo valid params found in formula below!!! Quitting.')
    print(fit_poly)
    sys.exit()

def _generate_constraints(fit_poly):
    out = {}
    for i in range(_get_nparams(fit_poly)):
        if i == 0:
    ...
    if fit_len > 3:
        if fit_name.endswith('C') or fit_name.endswith('CS'):
            ...
            if fit_len == 4 or (fit_len == 5 and fit_name.endswith('CS')):
                ...
                sys.exit()
        elif (fit_len == 4 and fit_name.endswith('M')) or (fit_len == 5 and fit_name.endswith('MS')):
            if verb: print('  * Replacing "y" with "((y+(10/52))/(x+(60/160)))" in fit function')
            fit_poly = fit_poly.replace('y','((y+(10/52))/(x+(60/160)))')
        elif fit_len == 4 and fit_name.endswith('S'):
            True  ## Do smearing below
        else:
            if verb: print('\n\n*** ERROR! Invalid fit function %s! Quitting. ***' % fit_name)
            sys.exit()
    if fit_name.endswith('S'):
        ## Replace "SMEAR" with smearing factor below
        fit_poly = fit_poly+'*SMEAR@'+str(_get_nparams(fit_poly))
...
def _get_rpf_options():
    ...
    return _rpf_options
## End function: _get_rpf_options()

def _shift_hist(in_hist, shift, fl):
    if shift == 'C':
        return in_hist
    if not in_hist.GetName().endswith('_'+fl+'_FULL'):
        print('\nIn _shift_hist, %s does not end in _%s_FULL!!! Quitting.' % (in_hist.GetName(), fl))
        sys.exit()
    out_hist = in_hist.Clone(in_hist.GetName().replace('_%s_FULL' % fl, '_%s_%s_FULL' % (shift, fl)))
    nX = out_hist.GetNbinsX()
    nY = out_hist.GetNbinsY()
    dX = 1 if 'R' in shift else (-1 if 'L' in shift else 0)
    dY = 1 if 'U' in shift else (-1 if 'D' in shift else 0)
    for iX in range(1, nX+1):
        for iY in range(1, nY+1):
            sX = max(1, min(nX, iX+dX))
            sY = max(1, min(nY, iY+dY))
            out_hist.SetBinContent(iX, iY, in_hist.GetBinContent(sX,sY))
            out_hist.SetBinError(iX, iY, in_hist.GetBinError(sX,sY))
    print('%s had integral %.1f, %s has %.1f' % (in_hist.GetName(), in_hist.Integral(), out_hist.GetName(), out_hist.Integral()))
    return out_hist
## End function: _shift_hist(in_hist, shift, fl)

def _smear_fit_form(formula, shift):
    if not 'SMEAR' in formula:
        if shift == 'C': return formula
    base = formula.split('SMEAR')[0]
    var = formula.split('SMEAR')[1]
    if len(var) != 2 or not var.startswith('@'):
        print('\n%s in %s (shift %s) does not look like "@1","@2", etc. Fix!!!' % (var, formula, shift))
        sys.exit()
    ## Can define tanh = (e^(2x) - 1) / (e^(2x) + 1) if needed
    if shift in ['U','D','L','R']:
        #return base+'(1/9.0)*(0.5+0.5*tanh('+var+'))'
        return base+'(1/9.0)*(0.5+0.5*('+var+'))'
    elif shift in ['UL','UR','DL','DR']:
        #return base+'(1/9.0)*(0.5+0.5*tanh('+var+'))*(0.5+0.5*tanh('+var+'))'
        return base+'(1/9.0)*(0.5+0.5*('+var+'))*(0.5+0.5*('+var+'))'
    elif shift == 'C':
        #return base+'(1.0 - (4/9.0)*(0.5+0.5*tanh('+var+'))*(1 + (0.5+0.5*tanh('+var+'))))'
        return base+'(1.0 - (4/9.0)*(0.5+0.5*('+var+'))*(1 + (0.5+0.5*('+var+'))))'
    else:
        print('\nInvalid shift %s!!! Quitting.' % shift)
        sys.exit()
## End function: _smear_fit_form(formula, shift)
...
def test_make(SRorCR):
    ...
    for ps, fl in [['Pass', 'Fail'] for r in twoD.ledger.GetRegions() if r == 'Pass']:
        ...
        ## Construct shifted "fail" distributions (up/down/left/right by one bin) for smearing
        fail_name = CAT+'Background_'+fl
        qcd_f = {}
        #shifts = ['C','U','D','L','R','UL','UR','DL','DR']
        shifts = ['C','U']
        for shift in shifts:
            fail_name_S = fail_name if shift == 'C' else fail_name.replace(fl, shift+'_'+fl)
            qcd_f[shift] = BinnedDistribution(
                fail_name_S, _shift_hist(qcd_hists[fl], shift, fl),
        ...
        twoD.AddAlphaObj(CAT+'Background', fl, qcd_f['C'])
        ...
        for opt_name in rpf_options.keys():
            ...
            qcd_rpf = {}
            qcd_p = None
            #shifts = ['C','U','D','L','R','UL','UR','DL','DR'] if opt_name.endswith('S') else ['C']
            shifts = ['C','U'] if opt_name.endswith('S') else ['C']
            for shift in shifts:
                fail_name_S = fail_name if shift == 'C' else fail_name.replace(fl, shift+'_'+fl)
                print(_smear_fit_form(opt_fit['form'], shift))

                qcd_rpf[shift] = ParametricFunction(
                    (fail_name_S.replace('Fail','rpfL')).replace('fail','rpfL')+'_'+opt_name,
                    binning_f, _smear_fit_form(opt_fit['form'], shift),
                ...
                qcd_p_name = fail_name.replace(fl, shift+'_'+fl).replace('Fail','Pass')+'_'+opt_name
                if shift == shifts[-1]:
                    qcd_p_name = fail_name.replace('Fail','Pass')+'_'+opt_name

                if shift == 'C' and not qcd_p:
                    qcd_p = qcd_f['C'].Multiply(qcd_p_name, qcd_rpf['C'])
                elif shift != 'C' and qcd_p:
                    qcd_p = qcd_p.Add(qcd_p_name, qcd_f[shift].Multiply(qcd_p_name+'_Mult', qcd_rpf[shift]))
            ## End loop: for shift in shifts
